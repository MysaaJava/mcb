...
is just a term, which is called a type when it occurs on the right-hand
side of a colon in a well-formed typing judgment, like $T$ in
...
For instance, in a context containing a variable \C{n} with
...
same term has type \C{bool}. In this case, it even computes and displays the
correct type:
...
% The context of a typing judgment logs not only the variables but
% also the facts currently assumed. Indeed an assumption like
% \C{p_pr} is a just a variable that has a statement as its type.
% 
% \begin{coq-left}{name=check-prime}{}
% Variable p : nat
% Variable p_pr : prime p
% Check prime_gt0 p p_pr : 0 < p.
% \end{coq-left}
% \begin{coqout-right}
% $~$
% $~$
% prime_gt0 p p_pr : 0 < p
% \end{coqout-right}
...
on the right-hand side of the colon symbol in a typing judgment. Term $T$ can be
...
equals does not behave the same for terms at the left of a colon, and
for those on the right --- types.


We assume a collection of distinct names, used to denote atomic terms
and called \emph{sorts}.  One of these sorts is called \C{Prop}, and it
...
is not necessarily a provable one:
...
the instance of \C{Type} on the right-hand side of the colon has a greater
...
\C{Set} happens to be a name for the smallest element of the
hierarchy of \C{Type}:
...
type of \C{g} is a binder, which allows one to describe the dependency in
...
integers, or an integer into rational numbers: working with these
...
bound in the body of the definition, the \C{n} on the right of the \C{:=}
...
an argument \C{(fun x => t) u} into $t[u/x]$: the formal argument
...
It is out of scope for this book to detail other uses of
...
the implementation of the proof command \C{ring} that decides
equalities in the
...
\end{coq}
introduces a new type \C{nat : Set} and two new terms \C{(O : nat)} and
\C{(S : nat -> nat)}. Constructors of a type \C{T} are function symbols,
possibly of zero arguments like \C{O}, and the codomain of their type is
always \C{T}. The inductive type \C{T} may occur in
the type of certain arguments of its constructors, like \C{nat}
in the type of
...
of the postfix  notations \C{n.+1} for term \C{(S n)}, \C{n.+2} for
term \C{(S (S n))}, etc. so as to display constructors 
...
\index[concept]{injectivity of constructors}
...
criterion. For instance, in the case of \C{addn}, the recursive call
...
using a conjunctive hypothesis. Here is the left elimination rule:
...
quantification indicates that \C{ex} and \C{ex_intro} are polymorphic
...
\C{ex_intro} constant features a last, innermost \C{forall a : A}
quantifier, which binds a term variable \C{a} representing the witness
of the existential statement for \C{P}.\\

We now look at the inductive definition of the disjunction \C{or}
...
prove the ``stronger'' induction principle that gives access, in
...
Through the lens of the proofs-as-programs correspondence, this means
